...
class LatticeBoostRegressor(RegressorMixin,BaseEstimator):
    ...
    def _prepare(self,train,others):
        ...
        # The decoded price becomes a feature of a second model. Cross-fit
        # its label-derived lattice on training rows too, even though the
        # real-data increment is identical in every fold.
        crossfit_price=np.empty(len(train))
        for a,b in KFold(5,shuffle=True,random_state=self.seed).split(train):
            inner=LatticeStoreRate()
            inner.price_increment_=float(np.gcd.reduce(np.rint(train.iloc[a][TARGET].to_numpy()*10000).astype(np.int64)))/10000
            crossfit_price[b]=inner.decode_price(train.iloc[b])
        decoded[0]=train.assign(Item_MRP=crossfit_price)
