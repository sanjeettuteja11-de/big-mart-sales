...
def run(train_raw: pd.DataFrame, test_raw: pd.DataFrame,
    ...
    from src.structure import recover_units, units_findings

    units, offset = recover_units(train[TARGET], train["Item_MRP"])
    fig, axes = plt.subplots(1, 2, figsize=(11, 4))
    sns.histplot(offset[~np.isnan(offset)], binwidth=0.1, binrange=(-2.05, 2.05), ax=axes[0])
    axes[0].set(title="Unit price minus MRP", xlabel="Offset")
    sns.boxplot(x=train["Outlet_Type"], y=units, fliersize=1, ax=axes[1])
    axes[1].set(title="Whole units sold per row, by store type", xlabel="", ylabel="Units")
    axes[1].tick_params(axis="x", rotation=15)
    _save(fig, figures, "08_units_and_price_offset")
    findings.extend(units_findings(train))
